...
test.describe('User Authentication Flow', () => {
    ...
    test('Should successfully login with valid credentials', async ({ }, testInfo) => {
        // Add launch attributes for better categorization 
        await testInfo.attach('Test Environment', {
            body: JSON.stringify({
                browser: testInfo.project.name,
                viewport: loginPage.page.viewportSize(),
                userAgent: await loginPage.page.evaluate(() => navigator.userAgent)
            }),
        ...
        await test.step('Navigate to login page', async () => {
            ...
            await testInfo.attach('Login Page Loaded', {
                body: await loginPage.page.screenshot({ fullPage: true }),
        ...
        await test.step('Verify successful authentication', async () => {
            await expect(inventoryPage.page.url()).toContain('/inventory.html');
