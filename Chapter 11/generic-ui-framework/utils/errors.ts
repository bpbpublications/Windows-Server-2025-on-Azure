...
export class ElementActionError extends Error {
  constructor(action: string, selector: string, originalError: any) {
    super(
      `Failed to ${action} on selector: ${selector}. Error: ${originalError?.message || originalError}`,
    );
...
export class NavigationError extends Error {
  constructor(url: string, originalError: any) {
    super(
      `Failed to navigate to URL: ${url}. Error: ${originalError?.message || originalError}`,
    );
